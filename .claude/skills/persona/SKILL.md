...
### Passo Final: Persistir no Projeto (OBRIGATORIO — ao final do Fluxo 6)

Se a conversa estiver rodando dentro de um projeto com acesso ao sistema de arquivos (Claude Code, ou qualquer superficie com permissao de escrita local), executar automaticamente, sem perguntar:

1. **Salvar o documento completo** em `docs/persona.md` na raiz do projeto (criar a pasta `docs/` se nao existir). Conteudo: a Consolidacao Final inteira, com todos os fluxos executados.
2. **Atualizar o `CLAUDE.md`** do projeto (criar se nao existir) adicionando ou substituindo uma secao `## Persona do meu negocio` com:
   - Nome ficticio
   - Top 3 dores mais profundas
   - Top 3 desejos mais intensos
   - Frase de qualificacao rapida
   - Uma linha: "Documento completo em docs/persona.md"

   Manter essa secao curta (nunca colar as 30 dimensoes inteiras no CLAUDE.md — ele deve ficar enxuto; o material extenso mora em `docs/persona.md`).
3. Se **nao** houver acesso ao sistema de arquivos (ex.: a conversa esta rodando num app sem projeto conectado), pular este passo silenciosamente e, ao final, avisar o usuario:
   > "Gerei sua persona completa acima. Se quiser guardar isso permanentemente no seu projeto, copie esse conteudo para uma conversa do Claude Code na pasta do projeto e peca para salvar em `docs/persona.md`, com um resumo no `CLAUDE.md`."

**Regra:** Nunca perguntar "quer que eu salve?" quando ha acesso ao sistema de arquivos — salvar e o comportamento padrao do Fluxo 6. Perguntar so adiciona friccao a um passo que deveria ser automatico.
